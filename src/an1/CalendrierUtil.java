...
public class CalendrierUtil {
	...
	public void refresh() {
		...
		for (int i = 0; i < 3; i++) {
			...
			unMois.nomMois = sb.substring(fp1.getBeginIndex(),
					fp1.getEndIndex())
					+ " "
					+ sb1.substring(fp3.getBeginIndex(), fp3.getEndIndex());
			...
			for (int j = 1; j <= Max; j++) {
				...
				jour.setNom(sb.substring(fp2.getBeginIndex(), fp2.getEndIndex())
						+ " " + j);
	...
	public CalendrierUtil(Date dateCentrale, ServicerefugeJpaController s,
	...
	public String getHTML(String[] selectedItems, boolean complet) {
		StringBuffer html = new StringBuffer();

		html.append("<table width=\"100%\" border=\"3\">");
		int nbreMois=3;
		if (!complet) nbreMois=1;
		for (int mois = 0; mois < nbreMois; mois++) {
			html.append("<tr><th style=\"font-size:22px; color:yellow\">"
					+ listeMois.get(mois).getNomMois() + "</th></tr>");
				...
				for (int jourSemaine = 1; jourSemaine <= 7; jourSemaine++) {
					...
					if (indexJourS == jourSemaine) {
						if (listeMois.get(mois).getListeJours().size() > indexJour) {
							UnJour jour = listeMois.get(mois).getListeJours()
									.get(indexJour);
							...
							html.append("<div align=\"center\" style =\"font-size:16px; font-weight:bold;\">"
									+ Integer.toString(indexJour) + "</div>");
							if (jour.getServiceRefuge().size() > 0) {

								for (int i = 0; i < jour.getServiceRefuge()
										.size(); i++)
									nombrePersonnesService += jour
											.getServiceRefuge().get(i)
											.getNombrePersonnes();
								...
								if (complet) {
									html.append("<img onClick=\"javascript:document.getElementById('form1:valeurMois').value='"
											+ mois
											+ "';document.getElementById('form1:valeurJour').value='"
											+ indexJour
											+ "';document.forms['form1'].submit();\" src=\"./resources/bonhomme.gif\"> ");
							...
							if (nbLits > 0
									|| jour.getReservations(selectedItems)
											.size() > 0) {
								if (complet)
									html.append("<img onClick=\"javascript:document.getElementById('form1:valeurMois').value='"
											+ mois
											+ "';document.getElementById('form1:valeurJour').value='"
											+ indexJour
											+ "';document.forms['form1'].submit();\" src=\"./resources/lit.png\"> ");

								for (int i = 0; i < jour.getReservations(
										selectedItems).size(); i++) {
									nbLits += jour
											.getReservations(selectedItems)
											.get(i).nombrePersonnes();
								}
							...
							if (!complet) {
								...
								if (nbLits >=45)
									color = "red";
								html.append("<div align=\"center\" style =\"background-color:"
										+ color + ";\">&nbsp</div>");
							}
						}
					}
	...
	/**
	 * @param reservationsEnCours
	 *            the reservationsEnCours to set
	 */
