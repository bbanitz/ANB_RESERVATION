...
//    private EntityManagerFactory emf = null;
    ...
    public void create(Reservations reservations) throws PreexistingEntityException, RollbackFailureException, Exception {
        ...
        try {
            ...
            if (findReservations(reservations.getId()) != null) {
                throw new PreexistingEntityException("Reservations " + reservations + " already exists.", ex);
            }
            throw ex;
        } finally {
            if (em != null) {
    ...
    public void edit(Reservations reservations) throws NonexistentEntityException, RollbackFailureException, Exception {
        EntityManager em = null;
        try {
            em = getEntityManager();
            utx = em.getTransaction();
    ...
    public int getReservationsCount() {
        EntityManager em = getEntityManager();
        try {
            return ((Long) em.createQuery("select count(o) from Reservations as o").getSingleResult()).intValue();
        } finally {
            em.close();
        }
    }
	...
	public List<Reservations> getReservationsMembre(Membres membre) {
        EntityManager em = getEntityManager();
        try {
            Query q = em.createQuery("select object(o) from Reservations as o where o.membre=:membre");
    ...
    @SuppressWarnings("unchecked")
	public List<Reservations> getReservations(Date jour){
        EntityManager em = getEntityManager();
        try {
            Query q = em.createQuery("select object(o) from Reservations as o where o.dateDebut<=:idDate and o.dateFin>=:idDate");
        ...
        for (String s:selectedValues) System.out.println(s);
        boolean premier=true;
        requete.append("select object(o) from Reservations as o where ");
        for (String selected:selectedValues) {
            if (premier) premier=false; else requete.append(" or ");
            if (selected.equals(Reservations.StatutReservation.Demande.toString()) )   requete.append("o.statut=\""+selected+"\"");
            else if (selected.equals(Reservations.StatutReservation.Option.toString()) )   requete.append("o.statut=\""+selected+"\"");
            else if (selected.equals(Reservations.StatutReservation.Ferme.toString()) )   requete.append("o.statut=\""+selected+"\"");
            else if (selected.equals(Reservations.StatutReservation.AcomptePayé.toString()) )   requete.append("o.statut=\""+selected+"\"");
            else if (selected.equals(Reservations.StatutReservation.Payé.toString()) )   requete.append("o.statut=\""+selected+"\"");
        }
